...
<aside id="main-sidebar" class="hidden lg:flex fixed left-0 top-0 bottom-0 w-[220px] flex-col border-r border-slate-200 bg-white/95 backdrop-blur-sm z-[55] overflow-y-auto">
    <div class="px-3 pt-3 pb-2 border-b border-slate-100">
        <a href="home.html" class="flex items-center gap-2 px-2 py-1.5">
            <div class="bg-brand-500 text-white p-1.5 rounded-lg shrink-0"><i class="ph-bold ph-buildings text-base"></i></div>
            <span class="sb-text text-[14px] font-extrabold text-slate-800 leading-tight">하남 지점<br><span class="text-[10px] font-bold text-slate-500">어드민</span></span>
        </a>
    </div>
    <div class="px-3 pt-4 pb-2 flex-1">
        <div class="px-2 pb-1.5 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider sb-label">메인 메뉴</div>
        <div class="space-y-0.5">
            <a href="home.html"          class="sb-link flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12.5px] font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors" title="대시보드 캘린더"><i class="ph ph-calendar-blank text-base shrink-0"></i><span class="sb-text">대시보드 캘린더</span></a>
            <a href="member.html"        class="sb-link flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12.5px] font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors" title="멤버 팀"><i class="ph ph-users-three text-base shrink-0"></i><span class="sb-text">멤버 팀</span></a>
            <a href="leader.html"        class="sb-link flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12.5px] font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors" title="리더 팀"><i class="ph ph-crown text-base shrink-0"></i><span class="sb-text">리더 팀</span></a>
            <a href="stats.html"         class="sb-link flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12.5px] font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors" title="통계 및 리포트"><i class="ph ph-chart-bar text-base shrink-0"></i><span class="sb-text">통계 및 리포트</span></a>
            <a href="call.html"          class="sb-link flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12.5px] font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors" title="신규 회신"><i class="ph ph-phone-call text-base shrink-0"></i><span class="sb-text">신규 회신</span></a>
            <a href="branchstatus.html"  class="sb-link flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12.5px] font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors" title="Branch Status"><i class="ph ph-building-office text-base shrink-0"></i><span class="sb-text">Branch Status</span></a>
            <a href="workdeck.html"      class="sb-link flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12.5px] font-medium text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors" title="Work Deck"><i class="ph ph-clipboard-text text-base shrink-0"></i><span class="sb-text">Work Deck</span></a>
        </div>
    </div>
    <div class="p-3 border-t border-slate-100">
        <button id="sb-toggle" onclick="document.body.classList.toggle('sb-collapsed');localStorage.setItem('admin.sbCollapsed',document.body.classList.contains('sb-collapsed')?'1':'0')" class="w-full flex items-center justify-center gap-2 py-2 rounded-lg text-[11px] font-bold text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-colors" title="사이드바 접기/펼치기">
            <i id="sb-toggle-icon" class="ph-bold ph-caret-double-left"></i>
            <span class="sb-text">접기</span>
        </button>
    </div>
</aside>
    ...
    <header class="glass-panel sticky top-0 z-50 px-4 py-3 flex items-center justify-between">
        <div class="flex items-center gap-4">
            <button id="hamburger-btn"
                class="p-1.5 text-slate-600 hover:bg-slate-100 rounded-lg cursor-pointer transition-colors">
        ...
        <div class="flex items-center gap-4">
            ...
            <div
                class="flex items-center gap-2 cursor-pointer hover:bg-slate-50 p-1.5 rounded-lg border border-transparent transition-colors">
                <img src="https://ui-avatars.com/api/?name=매니저&background=0284c7&color=fff" alt="User"
                    class="w-8 h-8 rounded-full">
                <span class="text-sm font-medium hidden sm:block">지점장 옵션 <i
                        class="ph ph-caret-down text-xs ml-1 font-bold"></i></span>
            </div>
        </div>
    </header>
    ...
    <div id="drawer-overlay"
        class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[60] hidden opacity-0 transition-opacity cursor-pointer">
    </div>
    <div id="nav-drawer"
        class="fixed top-0 left-0 h-full w-full sm:w-72 bg-white shadow-2xl z-[70] transform -translate-x-full transition-transform duration-300 flex flex-col">
        <div class="p-4 border-b border-slate-100 flex items-center justify-between bg-white">
            ...
            <button id="close-drawer-btn"
                class="p-2 text-slate-400 hover:bg-slate-200 rounded-full cursor-pointer transition">
                <i class="ph ph-x"></i>
            </button>
        </div>
        <nav class="p-4 space-y-1">
            <a href="home.html"
                class="nav-item w-full flex items-center gap-3 px-3 py-3 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-lg font-medium transition-colors">
                <i class="ph ph-calendar-blank text-lg"></i> 대시보드 캘린더 (홈)
            </a>
            <a href="member.html"
                class="nav-item w-full flex items-center gap-3 px-3 py-3 bg-brand-50 text-brand-700 rounded-lg font-medium transition-colors">
            ...
            <a href="branchstatus.html" class="nav-item w-full flex items-center gap-3 px-3 py-3 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-lg font-medium transition-colors">
                <i class="ph ph-building-office text-lg"></i> Branch Status
            </a>
            <a href="workdeck.html" class="nav-item w-full flex items-center gap-3 px-3 py-3 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-lg font-medium transition-colors">
                <i class="ph ph-clipboard-text text-lg"></i> Work Deck
            </a>
        </nav>
    </div>
    ...
    <div id="team-view">
        ...
        <nav id="team-tabs-nav" class="bg-white border-b border-slate-200 sticky z-40 h-[44px]" style="top:var(--tabs-top)">
            <div class="flex items-center gap-1.5 px-4 sm:px-6 lg:px-8 scrollbar-hide overflow-x-auto h-full">
                <!-- 전체 -->
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-slate-900 text-white text-[13px] font-bold shadow-sm transition-all active:scale-95">전체</button>
                ...
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-purple-600 border border-purple-200 hover:bg-purple-50 text-[13px] font-bold transition-all active:scale-95">전체</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-purple-600 border border-purple-200 hover:bg-purple-50 text-[13px] font-medium transition-all active:scale-95">월수 11</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-purple-600 border border-purple-200 hover:bg-purple-50 text-[13px] font-medium transition-all active:scale-95">월수 8</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-purple-600 border border-purple-200 hover:bg-purple-50 text-[13px] font-medium transition-all active:scale-95">화목 11</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-purple-600 border border-purple-200 hover:bg-purple-50 text-[13px] font-medium transition-all active:scale-95">화목 8</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-purple-600 border border-purple-200 hover:bg-purple-50 text-[13px] font-medium transition-all active:scale-95">토일 11</button>
                ...
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 text-[13px] font-bold transition-all active:scale-95">전체</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 text-[13px] font-medium transition-all active:scale-95">월수 11</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 text-[13px] font-medium transition-all active:scale-95">월수 8</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 text-[13px] font-medium transition-all active:scale-95">화목 11</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 text-[13px] font-medium transition-all active:scale-95">화목 8</button>
                <button class="team-tab flex-shrink-0 px-3 py-1 rounded-full bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 text-[13px] font-medium transition-all active:scale-95">토일 11</button>
            </div>
        </nav>

        <!-- Level Quick-Nav (scroll jump) -->
        <nav id="level-nav" class="bg-slate-50/90 backdrop-blur-sm border-b border-slate-200 sticky z-30 h-[44px]" style="top:var(--level-nav-top)">
            <div class="overflow-x-auto hide-scrollbar flex items-center gap-1.5 px-4 sm:px-6 lg:px-8 h-full">
                <span class="flex-shrink-0 text-[9px] font-bold text-slate-400 uppercase tracking-wider mr-1">바로가기</span>
                <!-- Level jump buttons injected by JS -->
            </div>
        </nav>

        <div class="px-4 sm:px-6 lg:px-8 pb-4 sm:pb-6 lg:pb-8 pt-0 space-y-6">
            <!-- Team Content Header & Toolbar -->
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-4">
                <div
                    class="flex items-center gap-2 group cursor-pointer transition-colors hover:bg-slate-50 p-1.5 -ml-1.5 rounded-lg">
                    <h2 id="team-title" class="text-2xl font-bold text-slate-800 flex flex-wrap items-center gap-1.5">
                        <span class="text-xl font-black text-slate-800 mr-1 italic">Member</span>
                        <!-- dynamic badges -->
                    </h2>
                    <button class="text-slate-300 group-hover:text-brand-600 p-1 transition-colors" title="팀 이름 수정">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="openSmsModal()"
                        class="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors shadow-sm focus:ring-2 focus:ring-slate-200">
                        <i class="ph ph-envelope-simple text-lg"></i> 일괄 문자
                    </button>
                </div>
            </div>


            <!-- Action Needed / Alerts Section (CHECK) -->
            <div id="check-section"
                class="bg-amber-50 rounded-xl shadow-sm border border-amber-200 overflow-hidden mb-4">
                <button onclick="toggleCheckSection()" class="w-full px-4 py-3 border-b border-amber-200 bg-amber-100/50 flex justify-between items-center cursor-pointer hover:bg-amber-100/80 transition-colors">
                    <h3 class="text-sm font-bold text-amber-800 flex items-center gap-2">
                        <i class="ph-fill ph-warning-circle text-lg"></i> 주의 알림 (CHECK)
                        <span id="check-count"
                            class="px-2 py-0.5 bg-amber-200 text-amber-800 rounded-full text-xs font-bold shadow-inner border border-amber-300">5명</span>
                    </h3>
                    <i class="ph ph-caret-down text-amber-400 text-sm transition-transform" id="check-collapse-icon"></i>
                </button>
                <div id="check-content-wrap" style="max-height:2000px;overflow:hidden;transition:max-height 0.4s cubic-bezier(0.4,0,0.2,1);">
                <div class="px-4 py-2 border-b border-amber-200 bg-amber-50 flex items-center gap-3">
                    <div class="flex items-center bg-white/50 rounded-lg p-0.5 gap-0.5 border border-amber-200">
                        <button id="alert-view-btn-list" onclick="setAlertViewMode('list')" title="리스트 보기"
                            class="w-7 h-7 rounded-md flex items-center justify-center transition-all bg-white shadow-sm text-amber-600">
                            <i class="ph-bold ph-list-bullets text-sm"></i>
                        </button>
                        <button id="alert-view-btn-card" onclick="setAlertViewMode('card')" title="카드 보기"
                            class="w-7 h-7 rounded-md flex items-center justify-center transition-all text-amber-400 hover:text-amber-600">
                            <i class="ph-bold ph-squares-four text-sm"></i>
                        </button>
                    </div>
                    <select id="inner-alert-filter" class="px-2 py-1 bg-white border border-amber-300 rounded text-xs font-medium text-amber-800 hover:bg-amber-50 focus:ring-2 focus:ring-amber-200 focus:border-amber-400 outline-none cursor-pointer">
                        <option value="all">전체 유형</option>
                        <option value="0">첫 스터디 미체크</option>
                        <option value="1">만료 D-7</option>
                        <option value="2">첫 스터디 체크</option>
                        <option value="3">결제 확인</option>
                        <option value="4">홀딩 복귀</option>
                    </select>
                </div>
                <div id="alert-list-container" class="divide-y divide-amber-100">
                    <!-- Alert Item 1 -->
                    <div class="p-3 hover:bg-amber-100/50 transition-colors group flex items-center justify-between gap-2 border-b border-amber-100/50"
                        id="alert-1">
                        <div class="grid grid-cols-[104px_72px_44px_90px_1fr] items-center gap-3 flex-1 min-w-0">
                            <!-- Badge -->
                            <span
                                class="inline-flex items-center justify-center w-full px-1.5 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700 border border-red-200 shadow-sm whitespace-nowrap"><span
                                    class="w-1.5 h-1.5 rounded-full bg-red-500 mr-1.5 animate-pulse"></span>첫방문
                                미체크</span>

                            <!-- Name -->
                            <button onclick="scrollToMember('member-row-1-0')"
                                class="text-sm font-bold text-slate-800 hover:text-brand-600 transition-colors flex items-center justify-between gap-1 w-full text-left">
                                김지윤 <i
                                    class="ph-bold ph-arrow-right text-[10px] opacity-0 group-hover:opacity-100 transition-opacity text-slate-400"></i>
                            </button>

                            <!-- Level -->
                            <span
                                class="inline-block w-full text-center px-1 py-0.5 rounded bg-slate-100 text-slate-600 text-[10px] font-semibold border border-slate-200">LV
                                2</span>

                            <!-- Phone -->
                            <span class="text-[11px] font-mono text-slate-600 text-center w-full cursor-pointer hover:text-brand-600 active:scale-95 transition-all" onclick="copyPhone('010-2757-9662')">010-2757-9662</span>

                            <!-- Team -->
                            <div class="hidden sm:flex items-center gap-1 overflow-hidden">
                                <span
                                    class="inline-block w-[88px] text-center px-1 py-0.5 text-[9px] rounded font-semibold border truncate whitespace-nowrap" style="color:#6C3483;background:#f3e8f9;border-color:#6C3483">영
                                    월수 8PM(2)</span>
                                <span
                                    class="inline-block w-[88px] text-center px-1 py-0.5 text-[9px] rounded font-semibold border truncate whitespace-nowrap" style="color:#2C3E50;background:#e8ecf0;border-color:#2C3E50">일
                                    화목 8PM(1)</span>
                            </div>
                        </div>

                        <!-- Resolve Button -->
                        <button onclick="resolveAlert('alert-1')"
                            class="shrink-0 p-1 px-2.5 rounded-lg text-amber-700 bg-white border border-amber-300 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-300 transition-all text-xs font-bold shadow-sm flex items-center gap-1">
                            <i class="ph-bold ph-check"></i> 해결
                        </button>
                    </div>
                    <!-- Alert Item 2 -->
                    <div class="p-3 hover:bg-amber-100/50 transition-colors group flex items-center justify-between gap-2"
                        id="alert-2">
                        <div class="grid grid-cols-[104px_72px_44px_90px_1fr] items-center gap-3 flex-1 min-w-0">
                            <!-- Badge -->
                            <span
                                class="inline-flex items-center justify-center w-full px-1.5 py-0.5 rounded text-[10px] font-bold bg-orange-100 text-orange-700 border border-orange-200 shadow-sm whitespace-nowrap"><i
                                    class="ph-fill ph-clock-countdown mr-1.5"></i>만료 D-7</span>

                            <!-- Name -->
                            <button onclick="scrollToMember('member-row-1-2')"
                                class="text-sm font-bold text-slate-800 hover:text-brand-600 transition-colors flex items-center justify-between gap-1 w-full text-left">
                                성경희 <i
                                    class="ph-bold ph-arrow-right text-[10px] opacity-0 group-hover:opacity-100 transition-opacity text-slate-400"></i>
                            </button>
                            ...
                            <!-- Phone -->
                            <span class="text-[11px] font-mono text-slate-600 text-center w-full cursor-pointer hover:text-brand-600 active:scale-95 transition-all" onclick="copyPhone('010-6385-8265')">010-6385-8265</span>

                            <!-- Team -->
                            <div class="hidden sm:flex items-center gap-1 overflow-hidden">
                                <span
                                    class="inline-block w-[88px] text-center px-1 py-0.5 text-[9px] rounded font-semibold border truncate whitespace-nowrap" style="color:#9B59B6;background:#f5eafa;border-color:#9B59B6">영
                                    월수 10AM(3)</span>
                            </div>
                        </div>

                        <!-- Resolve Button -->
                        <button onclick="resolveAlert('alert-2')"
                            class="shrink-0 p-1 px-2.5 rounded-lg text-amber-700 bg-white border border-amber-300 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-300 transition-all text-xs font-bold shadow-sm flex items-center gap-1">
                            <i class="ph-bold ph-check"></i> 해결
                        </button>
                    </div>
                </div>
                <!-- Card Grid View for Alerts -->
                <div id="alert-card-container" class="hidden p-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
                </div>
                </div><!-- end check-content-wrap -->
            </div>

            <!-- 검색 및 필터 바 -->
            <div class="flex flex-wrap sm:flex-nowrap items-center gap-2 mb-4">
                ...
                <select id="sort-select" class="px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 shadow-sm cursor-pointer">
                    <option value="att-desc">출석률 높은순</option>
                    <option value="att-asc">출석률 낮은순</option>
                    <option value="alert-first">주의 알림순</option>
                    <option value="first-day">당일 첫날 방문</option>
                    <option value="name-asc">이름 ㄱ→ㅎ</option>
                    <option value="name-desc">이름 ㅎ→ㄱ</option>
                    <option value="age-asc">나이 어린순</option>
                    <option value="age-desc">나이 많은순</option>
                    <option value="remain-asc">잔여 적은순</option>
                    <option value="remain-desc">잔여 많은순</option>
                    <option value="exp-asc">만료 임박순</option>
                    <option value="today-first">금일 출석 완료순</option>
                    <option value="today-last">금일 미출석순</option>
                </select>
                <span class="w-px h-6 bg-slate-200 mx-1 flex-shrink-0 hidden sm:block"></span>
                <select id="membership-filter" class="px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 shadow-sm cursor-pointer">
                ...
                <div class="flex items-center bg-slate-100 rounded-lg p-0.5 gap-0.5">
                    <button id="view-btn-table" onclick="setViewMode('table')" title="테이블 뷰"
                        class="w-8 h-8 rounded-md flex items-center justify-center transition-all bg-white shadow-sm text-brand-700">
                        <i class="ph-bold ph-list-bullets text-base"></i>
                    </button>
                    <button id="view-btn-card" onclick="setViewMode('card')" title="카드 그리드 뷰"
                        class="w-8 h-8 rounded-md flex items-center justify-center transition-all text-slate-500 hover:text-slate-700">
                        <i class="ph-bold ph-squares-four text-base"></i>
                    </button>
                </div>
                <button id="view-btn-builder" onclick="setViewMode('builder')" title="TEAM MAP — 수동 테이블 배치 뷰"
                    class="ml-2 px-3 h-8 rounded-lg flex items-center gap-1.5 text-[11px] font-black tracking-wider transition-all bg-white text-slate-600 border border-slate-200 hover:border-brand-300 hover:text-brand-700 hover:bg-brand-50 shadow-sm">
            ...
            <div id="bulk-action-bar"
                class="hidden items-center justify-between p-3 bg-brand-50 border border-brand-200 rounded-lg">
                ...
                <div class="flex items-center gap-2">
                    <button
                        class="px-3 py-1.5 bg-white border border-brand-200 text-brand-700 text-sm font-medium rounded hover:bg-brand-100 transition-colors flex items-center gap-1">
                    ...
                    <button class="p-1.5 text-red-500 hover:bg-red-50 rounded transition-colors" title="제외"><i
                            class="ph ph-trash text-lg"></i></button>
                </div>
            </div>

            <!-- List View Section -->
            <div id="view-list-section">
                <!-- Level Sections Container -->
                <div id="levels-container" class="space-y-6">
                    <!-- Each level section (header + table) injected by JS -->
                </div>
            </div>

            <!-- Card Grid Section -->
            <div id="view-card-section" class="hidden">
                <div id="levels-card-container" class="space-y-6">
                    <!-- Each level section (header + card grid) injected by JS -->
                </div>
            </div>

            <!-- Builder (Manual Table Layout) Section -->
            <div id="view-builder-section" class="hidden bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-4 py-3 border-b border-slate-200 bg-slate-50/80 flex items-center justify-between gap-3 flex-wrap">
                    <div class="flex items-center gap-2 text-sm font-bold text-slate-700">
                        <i class="ph-fill ph-table text-brand-500"></i>
                        <span>테이블 배치 뷰</span>
                        <span class="text-[10px] font-semibold text-slate-400 ml-1">현재 팀: <span id="builder-team-label" class="text-slate-700 font-bold">—</span></span>
                    </div>
                    <span id="builder-att-stats" class="text-[11px] font-bold text-slate-600 bg-white border border-slate-200 px-2 py-1 rounded-lg"><i class="ph-fill ph-check-square text-emerald-500 mr-0.5"></i>오늘 출석 <span data-att-stats="0/0">0/0</span></span>
                </div>
                <div id="builder-levels" class="p-4 space-y-6"></div>
            </div>

            <!-- Member Search Popover (used by Builder) -->
            <div id="bld-search-popover" class="hidden fixed z-[120] w-[320px] bg-white border border-slate-200 rounded-xl shadow-2xl overflow-hidden">
                <div class="px-3 py-2 border-b border-slate-100 flex items-center gap-2">
                    <i class="ph ph-magnifying-glass text-slate-400"></i>
                    <input id="bld-search-input" type="text" placeholder="이름 또는 전화번호 검색" class="flex-1 text-sm font-medium text-slate-800 outline-none placeholder-slate-300" autocomplete="off">
                    <button id="bld-search-clear" class="p-1 -m-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded transition-colors hidden"><i class="ph ph-x text-sm"></i></button>
                </div>
    ...
    <div id="global-block-tooltip"
        class="fixed hidden w-[140px] bg-slate-900 border border-slate-700 text-white rounded p-2 text-left z-[100] shadow-xl pointer-events-none transform -translate-x-1/2 -translate-y-full mt-[-4px]">
        <div id="gbt-date" class="text-[10px] text-slate-300 font-semibold mb-1 border-b border-slate-600 pb-1">04.05
            (월)</div>
        <div class="flex items-center justify-between mb-0.5 mt-1 text-[10px]"><span>상태:</span> <span id="gbt-status"
                class="font-bold">출석</span></div>
        <div class="flex items-center justify-between text-[10px]"><span>담당:</span> <span id="gbt-leader">박선민</span>
        </div>
        <!-- Tooltip arrow -->
        <div
            class="absolute top-full left-1/2 -translate-x-1/2 border-[5px] border-solid border-slate-900 border-b-transparent border-l-transparent border-r-transparent">
        </div>
    </div>
    ...
    <div id="profile-modal-overlay"
        class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[80] hidden opacity-0 transition-opacity duration-300"
        onclick="closeProfileModal()"></div>
    <div id="profile-modal"
        class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90%] sm:w-[500px] h-[85vh] sm:h-[800px] max-h-[90vh] bg-white rounded-2xl shadow-2xl z-[90] hidden opacity-0 scale-95 transition-all duration-300 flex flex-col overflow-hidden">

        <!-- Header: Profile Summary -->
        <div class="p-6 bg-slate-900 text-white relative flex-shrink-0">
            <button onclick="closeProfileModal()"
                class="absolute top-4 right-4 text-slate-400 hover:text-white p-1 rounded-full hover:bg-white/10 transition-colors">
                <i class="ph ph-x text-xl"></i>
            </button>
            <div class="flex gap-4 items-center">
                <div
                    class="h-16 w-16 bg-gradient-to-tr from-brand-400 to-indigo-500 rounded-full flex items-center justify-center shadow-inner text-2xl font-black border-2 border-white/20">
                    <span id="pm-initial">김</span>
                </div>
                <div>
                    <h2 class="text-2xl font-bold flex items-center gap-2">
                        <span id="pm-name">김지윤</span>
                        <span id="pm-level-badge"
                            class="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30">LV
                            2</span>
                        <span id="pm-membership-badge" class="px-2 py-0.5 rounded text-[10px] font-bold border">VVIP
                            멤버십</span>
                    </h2>
                    <div id="pm-phone" class="text-slate-300 text-sm mt-1 font-mono cursor-pointer hover:text-white active:scale-95 transition-all" onclick="copyPhone(this.innerText)">010-2757-9662</div>
        ...
        <div class="flex-1 overflow-y-auto bg-slate-50 p-6 custom-scrollbar hide-scrollbar">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                <!-- 최초 등록일 -->
                <div class="bg-white p-3.5 rounded-xl shadow-sm border border-slate-200 flex flex-col min-h-[96px]">
                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-auto">최초 등록일</div>
                    <div class="flex flex-col items-start gap-1">
                        <span id="pm-reg-date" class="font-bold text-slate-800 text-[14px] leading-none">25.10.05</span>
                        <span class="text-[10px] text-transparent select-none">-</span> <!-- Placeholder for alignment -->
                    </div>
                </div>
                <!-- 멤버십 & 횟수 -->
                <div class="bg-white p-3.5 rounded-xl shadow-sm border border-slate-200 flex flex-col min-h-[96px]">
                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-auto">멤버십 & 횟수</div>
                    <div class="flex flex-col items-start gap-1">
                        <span id="pm-brand-name" class="font-bold text-slate-800 text-[14px] leading-none">VVIP</span> 
                        <span id="pm-remain-session" class="bg-brand-50 text-brand-600 px-1.5 py-0.5 rounded text-[10px] font-semibold border border-brand-100 max-w-full truncate">82 / 1040회</span>
                    </div>
                </div>
                <!-- 시작 & 만료일 -->
                <div class="bg-white p-3.5 rounded-xl shadow-sm border border-slate-200 flex flex-col min-h-[96px]">
                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-auto">시작 & 만료일</div>
                    <div class="flex flex-col items-start gap-1">
                        <span id="pm-start-exp-date" class="font-bold text-slate-800 text-[13px] leading-none">25.10.05 ~ 26.04.09</span>
                        <span id="pm-exp-note" class="bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded text-[10px] font-semibold border border-orange-100 max-w-full truncate">주 4회 권한</span>
                    </div>
                </div>
                <!-- 누적 출석률 -->
                <div class="bg-white p-3.5 rounded-xl shadow-sm border border-slate-200 flex flex-col min-h-[96px]">
                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-auto">누적 출석률</div>
                    <div class="flex flex-col items-start gap-1">
                        <span id="pm-att-rate" class="font-bold text-slate-800 text-[14px] leading-none">25%</span> 
                        <span id="pm-att-count" class="bg-purple-50 text-purple-600 px-1.5 py-0.5 rounded text-[10px] font-semibold border border-purple-100 max-w-full truncate">4 / 16회 참석</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-indigo-50 border border-indigo-100 p-4 rounded-xl shadow-sm mb-6">
                <h3 class="text-xs font-bold text-indigo-800 mb-2 flex items-center gap-1"><i
                        class="ph-fill ph-chat-teardrop text-base"></i> 관리자 상담 코멘트</h3>
        ...
        <div class="p-4 border-t border-slate-200 bg-white flex gap-2 justify-end flex-shrink-0">
            <button
                class="px-4 py-2 text-sm font-semibold text-slate-600 hover:text-slate-900 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors">정보
                수정</button>
            <button
                class="px-4 py-2 text-sm font-bold text-white bg-brand-600 rounded-lg shadow-sm hover:bg-brand-700 transition-colors flex items-center gap-1.5"><i
                    class="ph-bold ph-paper-plane-right"></i> 개별 문자</button>
        </div>
    </div>

    <!-- Attendance History Detail Modal -->
    <div id="ah-modal-overlay" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] hidden opacity-0 transition-opacity duration-300" onclick="closeMemberAttendanceHistory()"></div>
    <div id="ah-modal" class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] sm:w-[500px] max-h-[85vh] bg-slate-50 rounded-2xl shadow-2xl z-[110] hidden opacity-0 translate-y-4 transition-all duration-300 flex flex-col overflow-hidden border border-white">
        <!-- Header -->
        <div class="p-5 bg-white border-b border-slate-200 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="p-2.5 bg-brand-50 rounded-xl text-brand-600 shadow-sm border border-brand-100">
                    <i class="ph-bold ph-calendar-check text-2xl"></i>
                </div>
                <div>
                    <h3 class="text-base font-bold text-slate-800 flex items-center gap-2">
                        <span id="ah-modal-name"></span>
                        <span class="text-xs font-normal text-slate-400">멤버</span>
                    </h3>
                    <p class="text-[11px] text-brand-600 font-bold uppercase tracking-wider mt-0.5">전체 출석 히스토리</p>
                </div>
            </div>
            <button onclick="closeMemberAttendanceHistory()" class="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors group">
                <i class="ph ph-x text-xl group-hover:rotate-90 transition-transform"></i>
            </button>
        </div>

        <!-- Cumulative Stats Panel -->
        <div class="px-5 py-4 bg-brand-50/50 border-b border-slate-100 flex items-center justify-around">
            <div class="text-center">
                <div class="text-[10px] font-bold text-slate-400 uppercase mb-1">총 누적 %</div>
                <div id="ah-modal-total-rate" class="text-xl font-black text-brand-700">0%</div>
            </div>
            <div class="w-px h-8 bg-slate-200"></div>
            <div class="text-center">
                <div class="text-[10px] font-bold text-slate-400 uppercase mb-1">진행 횟수</div>
                <div id="ah-modal-total-count" class="text-xl font-black text-slate-800">0/0회</div>
            </div>
            <div class="w-px h-8 bg-slate-200"></div>
            <div class="text-center">
                <div class="text-[10px] font-bold text-slate-400 uppercase mb-1">상태</div>
                <div id="ah-modal-status" class="text-[13px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200">활동 중</div>
            </div>
        </div>
        
        <!-- List Body -->
        <div id="ah-modal-list" class="p-5 space-y-3.5 overflow-y-auto bg-slate-50 custom-scrollbar flex-1">
            <!-- JS fills -->
        </div>

        <!-- Footer -->
        <div class="p-4 bg-white border-t border-slate-200 text-center flex-shrink-0">
            <button onclick="closeMemberAttendanceHistory()" class="w-full py-2.5 bg-slate-900 text-white text-sm font-bold rounded-xl hover:bg-slate-800 transition-all shadow-lg active:scale-[0.98]">확인</button>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function tDrawer(s) {
                if (s) { dOverlay.classList.remove('hidden'); setTimeout(() => { dOverlay.classList.remove('opacity-0'); nDrawer.classList.remove('-translate-x-full'); }, 10); }
                else { dOverlay.classList.add('opacity-0'); nDrawer.classList.add('-translate-x-full'); setTimeout(() => dOverlay.classList.add('hidden'), 300); }
            }
            ...
            window.closeProfileModal = function() {
                pOverlay.classList.add('opacity-0'); pModal.classList.add('opacity-0', 'scale-95');
                setTimeout(() => { pOverlay.classList.add('hidden'); pModal.classList.add('hidden'); }, 300);
            };
